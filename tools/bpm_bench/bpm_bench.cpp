...
void ModifyPage(char *data, size_t page_idx, uint64_t seed) {
  auto *pg = reinterpret_cast<QalshBenchPageHeader *>(data);
  pg->seed_ = seed;
...
void CheckPageConsistentNoSeed(const char *data, size_t page_idx){
  const auto *pg = reinterpret_cast<const QalshBenchPageHeader *>(data);
...
// Check the page and verify the data inside
void CheckPageConsistent(const char *data, size_t page_idx, uint64_t seed) {
  const auto *pg = reinterpret_cast<const QalshBenchPageHeader*>(data);
  if (pg->seed_ != seed) {
