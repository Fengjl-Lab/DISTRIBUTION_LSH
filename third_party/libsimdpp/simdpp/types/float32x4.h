...
public:
    ...
    SIMDPP_INL float float32<4>(const float32<4> &) = default;
    SIMDPP_INL float32<4> &operator=(const float32<4> &) = default;

    template<class E> SIMDPP_INL float float32<4>(const float32<4, E>& d) { *this = d.eval(); }
    ...
    SIMDPP_INL float32<4>(float d) : d_(d) {}
